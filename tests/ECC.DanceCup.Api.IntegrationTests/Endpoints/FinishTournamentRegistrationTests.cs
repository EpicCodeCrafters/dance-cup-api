...
public class FinishTournamentRegistrationTests : IClassFixture<DanceCupApiFactory>
{
    private readonly HttpClient _client;
    private readonly string _postgresConnectionString;

    public FinishTournamentRegistrationTests(DanceCupApiFactory factory)
    {
    ...
    [Fact]
    public async Task FinishTournamentRegistration_ShouldChangeStateToRegistrationFinished()
    {
        // Arrange
        ...
        await connection.ExecuteAsync(
            ...
            (1, now(), now(), (select "id" from "users" where "external_id" = 600), 'Test Tournament', 'Test Description', now() + interval '30 days', 'RegistrationInProgress', now() - interval '5 days', null, null, null);
            """
        );

        var tournamentId = await connection.QuerySingleAsync<long>(
            """
            select "id" from "tournaments" where "name" = 'Test Tournament' and "state" = 'RegistrationInProgress';
            """
        );
        ...
        var request = new FinishTournamentRegistrationRequest
        {
        ...
        await danceCupApiClient.FinishTournamentRegistrationAsync(request);
        ...
        tournament.State.Should().Be("RegistrationFinished");
        tournament.Version.Should().Be(2);
        tournament.RegistrationFinishedAt.Should().NotBeNull();
        tournament.RegistrationFinishedAt.Should().BeAfter(testStartedAt).And.BeBefore(DateTime.UtcNow);
        tournament.ChangedAt.Should().BeAfter(testStartedAt).And.BeBefore(DateTime.UtcNow);
    }

    [Fact]
    public async Task FinishTournamentRegistration_WhenTournamentNotFound_ShouldReturnError()
    {
        // Arrange
        ...
        var exception = await Assert.ThrowsAsync<Grpc.Core.RpcException>(
            async () => await danceCupApiClient.FinishTournamentRegistrationAsync(request)
        );
    ...
    [Fact]
    public async Task FinishTournamentRegistration_WhenNotInProgress_ShouldReturnError()
    {
        // Arrange
        ...
        await connection.ExecuteAsync(
            """
            insert into "users" ("version", "created_at", "changed_at", "external_id", "username") values
            (1, now(), now(), 601, 'testuser601');
            
            insert into "tournaments" ("version", "created_at", "changed_at", "user_id", "name", "description", "date", "state", "registration_started_at", "registration_finished_at", "started_at", "finished_at") values
            (1, now(), now(), (select "id" from "users" where "external_id" = 601), 'Not In Progress', 'Test', now() + interval '30 days', 'Created', null, null, null, null);
            """
        );

        var tournamentId = await connection.QuerySingleAsync<long>(
            """
            select "id" from "tournaments" where "name" = 'Not In Progress';
            """
        );
